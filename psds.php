...
echo "</h1>";
$abfrage_webby = "SELECT webby, COUNT(id) as anzahl FROM gfx WHERE typ = 'PSD' GROUP BY webby";
...
if (isset($_REQUEST["webby"]) && $_REQUEST["webby"] != "") {
    pagemittypundwebby($itemsPerPage, "gfx", "PSD", save($_REQUEST["webby"], $link), $link);
...
if ($start >= 0) {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'PSD' " . $zusatz . " ORDER BY id DESC LIMIT 
			" . $start . ", " . $itemsPerPage;
} else {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'PSD' " . $zusatz . " ORDER BY id DESC LIMIT 
			0, " . $itemsPerPage;
...
while ($row_icons = mysqli_fetch_object($ergebnis_icons)) {
    ...
    echo "<div class='gfx'>";
    echo "<img src='Psds/" . $row_icons->id . $row_icons->bild . "' >";
    echo "</div></center>";
    ...
    echo "</div>";
    echo "<center>";
    echo "<a href='psds/" . $row_icons->id . $row_icons->down . "' target='_blank'><img src='webicons/save4.png' border='0'></a>";
    echo "</center>";
