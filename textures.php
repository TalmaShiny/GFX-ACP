...
$abfrage_webby = "SELECT webby, COUNT(id) as anzahl FROM gfx WHERE typ = 'Texture' GROUP BY webby";
...
if (isset($_REQUEST["serie"]) && $_REQUEST["serie"] != "") {
    pagemittypundserie($itemsPerPage, "gfx", "Texture", save($_REQUEST["serie"], $link), $link);
    $zusatz = "AND serie = '" . save($_REQUEST["serie"], $link) . "'";
} elseif (isset($_REQUEST["webby"]) && $_REQUEST["webby"] != "") {
    pagemittypundwebby($itemsPerPage, "gfx", "Texture", save($_REQUEST["webby"], $link), $link);
...
if ($start >= 0) {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'Texture' " . $zusatz . " ORDER BY id DESC LIMIT 
			" . $start . ", " . $itemsPerPage;
} else {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'Texture' " . $zusatz . " ORDER BY id DESC LIMIT 
			0, " . $itemsPerPage;
...
while ($row_icons = mysqli_fetch_object($ergebnis_icons)) {
    ...
    echo "<div class='gfx'>";
    echo "<img src='Textures/" . $row_icons->id . "_vorschau" . $row_icons->vorschau . "'>";
    echo "</div></center>";
    ...
    echo "(" . $werte['anzahl'] . ")";
    echo "</div>";
    echo "<center><a href='big.php?id=" . $row_icons->id . "&image=Textures/" . $row_icons->id . $row_icons->bild . "' target='_blank'><img src='webicons/view1.png' border='0'></a></center>";
