...
$abfrage_webby = "SELECT webby, COUNT(id) as anzahl FROM gfx WHERE typ = 'Design' GROUP BY webby";
...
if (isset($_REQUEST["serie"]) && $_REQUEST["serie"] != "") {
    pagemittypundserie($itemsPerPage, "gfx", "Design", save($_REQUEST["serie"], $link), $link);
    $zusatz = "AND serie = '" . save($_REQUEST["serie"], $link) . "'";
} elseif (isset($_REQUEST["webby"]) && $_REQUEST["webby"] != "") {
    pagemittypundwebby($itemsPerPage, "gfx", "Design", save($_REQUEST["webby"], $link), $link);
...
if ($start >= 0) {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'Design' " . $zusatz . " ORDER BY id DESC LIMIT 
			" . $start . ", " . $itemsPerPage;
} else {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'Design' " . $zusatz . " ORDER BY id DESC LIMIT 
			0, " . $itemsPerPage;
...
while ($row_icons = mysqli_fetch_object($ergebnis_icons)) {
    ...
    echo "<div class='gfx'>";
    echo "<img src='Designs/" . $row_icons->id . "_vorschau" . $row_icons->vorschau . "'";
    ?>
    onmouseover="Tip('<img
            src=\'Designs/<?= $row_icons->id; ?>_vorschaugross<?= $row_icons->vorschaugross; ?>\'>')" onmouseout="UnTip()"
    <?php
    echo ">";
    echo "</div></center>";
    ...
    echo "</div>";
    echo "<center>";
    echo "<a href='Designs/" . $row_icons->id . "_view" . $row_icons->bild . "' target='_blank'>
				<img src='webicons/view6.png' border='0'></a> ";
    echo "<a href='Designs/" . $row_icons->id . "_download" . $row_icons->down . "' target='_blank'><img src='webicons/save4.png' border='0'></a>";
    echo "</center>";
