...
if (isset($_SESSION["acp"])) {
    ...
    if (!empty($_REQUEST["action"]) && $_REQUEST["action"] == "edit") {
        if (is_numeric($_REQUEST["id"])) {
            if (!empty($_POST["series"]) && !empty($_POST["name"])) {
                ...
                echo "<p class='ok'>Signature erfolgreich editiert.<br></p>";
                echo '<meta http-equiv="refresh" content="1; url=Signaturesverwalten.php">';
            } else {
    ...
    echo "<h1>Signature bearbeiten</h1>";
    ...
    echo "<form action='signaturesverwalten_edit.php?action=edit&id=" . $_REQUEST["id"] . "' method='POST'>";
    echo "<table>";
    echo "<tr>";
    echo "<td>Name:</td>";
    echo "<td>";
