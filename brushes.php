...
$abfrage_webby = "SELECT webby, COUNT(id) as anzahl FROM gfx WHERE typ = 'Brush' GROUP BY webby";
...
if (isset($_REQUEST["serie"]) && $_REQUEST["serie"] != "") {
    pagemittypundserie($itemsPerPage, "gfx", "Brush", save($_REQUEST["serie"], $link), $link);
    $zusatz = "AND serie = '" . save($_REQUEST["serie"], $link) . "'";
} elseif (isset($_REQUEST["webby"]) && $_REQUEST["webby"] != "") {
    pagemittypundwebby($itemsPerPage, "gfx", "Brush", save($_REQUEST["webby"], $link), $link);
...
if ($start >= 0) {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'Brush' " . $zusatz . " ORDER BY id DESC LIMIT 
			" . $start . ", " . $itemsPerPage;
} else {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'Brush' " . $zusatz . " ORDER BY id DESC LIMIT 
			0, " . $itemsPerPage;
...
while ($row_icons = mysqli_fetch_object($ergebnis_icons)) {
    ...
    echo "<div class='gfx'>";
    echo "<img src='Brushes/" . $row_icons->id . $row_icons->bild . "'";
    ?>
    onmouseover="Tip('<img
            src=\'Brushes/<?= $row_icons->id; ?>_vorschau<?= $row_icons->vorschau; ?>\'>')" onmouseout="UnTip()"
    <?php
    echo ">";
    echo "</div></center>";
    ...
    echo "</div>";
    echo "<center>";
    echo "<a href='Brushes/" . $row_icons->id . "_vorschau" . $row_icons->vorschau . "' target='_blank'>
				<img src='webicons/view6.png' border='0'></a> ";
    echo "<a href='Brushes/" . $row_icons->id . "_download" . $row_icons->down . "' target='_blank'><img src='webicons/save4.png' border='0'></a>";
    echo "</center>";
