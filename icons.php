...
$abfrage_webby = "SELECT webby, COUNT(id) as anzahl FROM gfx WHERE typ = 'Icon' GROUP BY webby";
...
if (isset($_REQUEST["serie"]) && $_REQUEST["serie"] != "") {
    pagemittypundserie($itemsPerPage, "gfx", "Icon", save($_REQUEST["serie"], $link), $link);
    $zusatz = "AND serie = '" . save($_REQUEST["serie"], $link) . "'";
} elseif (isset($_REQUEST["webby"]) && $_REQUEST["webby"] != "") {
    pagemittypundwebby($itemsPerPage, "gfx", "Icon", save($_REQUEST["webby"], $link), $link);
...
if ($start >= 0) {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'Icon' " . $zusatz . " ORDER BY id DESC LIMIT 
			" . $start . ", " . $itemsPerPage;
} else {
    $abfrage_icons = "SELECT * FROM gfx WHERE typ = 'Icon' " . $zusatz . " ORDER BY id DESC LIMIT 
			0, " . $itemsPerPage;
...
while ($row_icons = mysqli_fetch_object($ergebnis_icons)) {
    ...
    echo "<div class='gfx'>";
    echo "<div class='gfx'>";
    echo "<img src='Icons/" . $row_icons->id . $row_icons->bild . "'>";
    echo "</div>";
    echo "Von: " . $row_icons->webby . "<br />";
    echo "Am: " . date("d.m.y", $row_icons->timestamp) . "<br />";
    echo "Serie: " . $row_icons->serie . "<p/>";
